...
export default function BillingAddressUpdateForm(props) {
  const {
    id: idProp,
    billingAddress,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = billingAddressRecord
      ? { ...initialValues, ...billingAddressRecord }
      : initialValues;
  ...
  const [billingAddressRecord, setBillingAddressRecord] =
    React.useState(billingAddress);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(BillingAddress, idProp)
        : billingAddress;
      setBillingAddressRecord(record);
    };
    queryData();
  }, [idProp, billingAddress]);
  React.useEffect(resetStateValues, [billingAddressRecord]);
  const validations = {
    unitNumber: [],
    streetNumber: [],
    addressLine1: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            BillingAddress.copyOf(billingAddressRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BillingAddressUpdateForm")}
      {...rest}
    >
      <TextField
        label="Unit number"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || billingAddress)}
