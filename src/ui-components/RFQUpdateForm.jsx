...
export default function RFQUpdateForm(props) {
  const {
    id: idProp,
    rFQ,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = rFQRecord
      ? { ...initialValues, ...rFQRecord }
      : initialValues;
    setQuotationNumber(cleanValues.quotationNumber);
    setDateSent(cleanValues.dateSent);
    setCustRefNum(cleanValues.custRefNum);
    setCompanyName(cleanValues.companyName);
    setContact(cleanValues.contact);
    setPhone(cleanValues.phone);
    setEmail(cleanValues.email);
    setShippingTerms(cleanValues.shippingTerms);
    setShippingMethod(cleanValues.shippingMethod);
    setLeadTime(cleanValues.leadTime);
    setPaymentTerms(cleanValues.paymentTerms);
    setDueDate(cleanValues.dueDate);
    setQuantityRequested(cleanValues.quantityRequested);
    setQuantityQuoted(cleanValues.quantityQuoted);
    setUom(cleanValues.uom);
    setPrice(cleanValues.price);
    setDiscount(cleanValues.discount);
    setAttr1(cleanValues.attr1);
    setAttr2(cleanValues.attr2);
    setAttr3(cleanValues.attr3);
    setLineTotal(cleanValues.lineTotal);
    setSubtotal(cleanValues.subtotal);
    setSalesTax(cleanValues.salesTax);
    setTotal(cleanValues.total);
    setInternalComments(cleanValues.internalComments);
    setEmailComments(cleanValues.emailComments);
    setAttr4(cleanValues.attr4);
    setAttr5(cleanValues.attr5);
    setAttr6(cleanValues.attr6);
    ...
    setReceivingCompanyID(cleanValues.receivingCompanyID);
    setUserDetailsID(cleanValues.userDetailsID);
    setSendingCompanyID(cleanValues.sendingCompanyID);
    setErrors({});
  };
  const [rFQRecord, setRFQRecord] = React.useState(rFQ);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(RFQ, idProp) : rFQ;
      setRFQRecord(record);
    };
    queryData();
  }, [idProp, rFQ]);
  React.useEffect(resetStateValues, [rFQRecord]);
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "RFQUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        ...
        >
          <Button
            ...
            variation="primary"
            isDisabled={
              !(idProp || rFQ) || Object.values(errors).some((e) => e?.hasError)
            }
