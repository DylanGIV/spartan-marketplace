...
export default function ShippingAddressUpdateForm(props) {
  const {
    id: idProp,
    shippingAddress,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = shippingAddressRecord
      ? { ...initialValues, ...shippingAddressRecord }
      : initialValues;
    ...
    setRegi(cleanValues.regi);
    setPostalCode(cleanValues.postalCode);
    setCountryName(cleanValues.countryName);
    setErrors({});
  };
  const [shippingAddressRecord, setShippingAddressRecord] =
    React.useState(shippingAddress);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ShippingAddress, idProp)
        : shippingAddress;
      setShippingAddressRecord(record);
    };
    queryData();
  }, [idProp, shippingAddress]);
  React.useEffect(resetStateValues, [shippingAddressRecord]);
  const validations = {
    unitNumber: [],
    streetNumber: [],
    addressLine1: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ShippingAddress.copyOf(shippingAddressRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ShippingAddressUpdateForm")}
      {...rest}
    >
      <TextField
        label="Unit number"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || shippingAddress)}
